...
suite("upstream suffixed-numbers", () => {
	const testCasesFolder = new URL(
		"upstream-suffixed-numbers/tests/test_cases/",
		import.meta.url,
	);
	...
	function roundTrip(text) {
		return format(parse(text, options));
	}
	...
	function parseAndFormat(text) {
		const document = parse(text, options);
	...
	/** @type {[name: string, input: string, expectedOutput: string][]} */
	const validTestCases = [];
	/** @type {[name: string, input: Buffer][]} */
	...
	for (const testCase of readdirSync(new URL("input", testCasesFolder))) {
		...
		if (!existsSync(expectedOutputFile)) {
			...
			invalidTestCases.push([testCase, input]);
		} else {
			...
			validTestCases.push([testCase, input, expectedOutput]);
		}
	}

	suite("valid documents", () => {
		for (const [name, input, expectedOutput] of validTestCases) {
			test(name, () => {
				if (knownBrokenTests.has(name)) {
					assert.throws(() => {
	...
	suite("invalid documents", () => {
		for (const [name, input] of invalidTestCases) {
			test(name, () => {
				if (knownBrokenTests.has(name)) {
					assert.doesNotThrow(() => parse(input, options));
				} else {
					assert.throws(() => parse(input, options));
				}
			});
		}
	});
